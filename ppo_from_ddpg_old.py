...
#from torch.distributions.multivarite_normal import MultivariateNormal
...
class PPO_FROM_DDPG(object):
    ...
    def update(self):
        ...
        #batch_next_values = self.critic(batch_states, self.totensor([batch_actions.tolist()]))
        ...
        action_std = 0.5
        action_var = torch.full((self.n_actions,), action_std * action_std)
        action_var = action_var.expand_as(batch_actions)
        cov_mat = torch.diag_embed(action_var)
        dist = MultivariateNormal(batch_actions, cov_mat)
        action_logprobs = dist.log_prob(batch_actions)

        action_var = torch.full((self.n_actions,), action_std * action_std)
        action_var = action_var.expand_as(target_next_actions)
        cov_mat = torch.diag_embed(action_var)
        dist = MultivariateNormal(target_next_actions, cov_mat)
        target_action_logprobs = dist.log_prob(target_next_actions)

        ratios = torch.exp(target_action_logprobs - action_logprobs.detach())
        some_rewards = []
        discounted_reward = 0
	for reward, is_terminal in zip(reversed(batch_rewards), reversed(terminates)):
            if is_terminal:
                discounted_reward = 0
            discounted_reward = reward + (self.gamma * discounted_reward)
            some_rewards.insert(0, discounted_reward)


	some_rewards = torch.tensor(some_rewards)
	some_rewards = (some_rewards - some_rewards.mean()) / (some_rewards.std() + 1e-5)

        advantages = some_rewards - current_value.detach()

        surr1 = ratios * advantages
        surr2 = torch.clamp(ratios, 1 - self.eps_clip, 1 + self.eps_clip) * advantages
        loss = -torch.min(surr1, surr2) + 0.5 * self.MseLoss(current_value, some_rewards) - 0.01 * dist.entropy()
        # Update Critic
